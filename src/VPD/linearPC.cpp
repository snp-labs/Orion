...
__hhash_digest *commit(const prime_field::field_element *src, long long N) {
  ...
  for (int i = 0; i < column_size; ++i) {
    ...
    memcpy(coef[i], &src[i * N / column_size],
           sizeof(prime_field::field_element) * N / column_size);
    memset(encoded_codeword[i], 0,
           sizeof(prime_field::field_element) * N / column_size * 2);
    codeword_size[i] =
        encode(&src[i * N / column_size], encoded_codeword[i], N / column_size);
  }
  ...
  for (int i = 0; i < N / column_size * 2; ++i) {
    ...
    for (int j = 0; j < column_size / 2; ++j) {
      stash[i] = merkle_tree::hash_double_field_element_merkle_damgard(
          encoded_codeword[2 * j][i], encoded_codeword[2 * j + 1][i], stash[i]);
    }
  }

  merkle_tree::merkle_tree_prover::create_tree(stash, N / column_size * 2, mt,
...
std::pair<long long, long long> prepare_enc_count(long long input_size,
                                                  long long output_size_so_far,
                                                  int depth) {
  ...
  auto output_depth_output_size =
      prepare_enc_count(C[depth].R, output_size_so_far + input_size, depth + 1);
  gates_count[output_depth_output_size.first + 1] =
      output_depth_output_size.second + D[depth].R;
  return std::make_pair(output_depth_output_size.first + 1,
                        gates_count[output_depth_output_size.first + 1]);
}
...
void prepare_gates_count(long long *query, long long N, int query_count) {
  ...
  std::pair<long long, long long> output_depth_output_size =
      prepare_enc_count(C[0].R, N, 1);

  gates_count[output_depth_output_size.first + 1] =
      N + output_depth_output_size.second + D[0].R;
  gates_count[output_depth_output_size.first + 2] = query_count;
}

std::pair<long long, long long>
generate_enc_circuit(long long input_size, long long output_size_so_far,
                     long long recursion_depth, long long input_depth) {
  if (input_size <= distance_threshold) {
  ...
  v.C.circuit[input_depth + 1].src_expander_C_mempool =
      new int[cn * C[recursion_depth].L];
  v.C.circuit[input_depth + 1].weight_expander_C_mempool =
      new prime_field::field_element[cn * C[recursion_depth].L];
  int mempool_ptr = 0;
  for (int i = 0; i < C[recursion_depth].R; ++i) {
    int neighbor_size = C[recursion_depth].r_neighbor[i].size();
    v.C.circuit[input_depth + 1].gates[output_size_so_far + i].ty =
        gate_types::custom_linear_comb;
    v.C.circuit[input_depth + 1]
        .gates[output_size_so_far + i]
        .parameter_length = neighbor_size;
    v.C.circuit[input_depth + 1].gates[output_size_so_far + i].src =
        &v.C.circuit[input_depth + 1].src_expander_C_mempool[mempool_ptr];
    v.C.circuit[input_depth + 1].gates[output_size_so_far + i].weight =
        &v.C.circuit[input_depth + 1].weight_expander_C_mempool[mempool_ptr];
    ...
    for (int j = 0; j < neighbor_size; ++j) {
      v.C.circuit[input_depth + 1].gates[output_size_so_far + i].src[j] =
          C[recursion_depth].r_neighbor[i][j] + C_input_offset;
      v.C.circuit[input_depth + 1].gates[output_size_so_far + i].weight[j] =
          C[recursion_depth].r_weight[i][j];
    }
  }

  auto output_depth_output_size = generate_enc_circuit(
      C[recursion_depth].R, output_size_so_far + C[recursion_depth].R,
      recursion_depth + 1, input_depth + 1);
  ...
  v.C.circuit[final_output_depth].src_expander_D_mempool =
      new int[dn * D[recursion_depth].L];
  v.C.circuit[final_output_depth].weight_expander_D_mempool =
      new prime_field::field_element[dn * D[recursion_depth].L];

  for (long long i = 0; i < D[recursion_depth].R; ++i) {
    long long neighbor_size = D[recursion_depth].r_neighbor[i].size();
    v.C.circuit[final_output_depth].gates[output_size_so_far + i].ty =
        gate_types::custom_linear_comb;
    v.C.circuit[final_output_depth]
        .gates[output_size_so_far + i]
        .parameter_length = neighbor_size;
    v.C.circuit[final_output_depth].gates[output_size_so_far + i].src =
        &v.C.circuit[final_output_depth].src_expander_D_mempool[mempool_ptr];
    v.C.circuit[final_output_depth].gates[output_size_so_far + i].weight =
        &v.C.circuit[final_output_depth].weight_expander_D_mempool[mempool_ptr];
    mempool_ptr += D[recursion_depth].r_neighbor[i].size();
    for (long long j = 0; j < neighbor_size; ++j) {
      v.C.circuit[final_output_depth].gates[output_size_so_far + i].src[j] =
          D[recursion_depth].r_neighbor[i][j] + D_input_offset;
      v.C.circuit[final_output_depth].gates[output_size_so_far + i].weight[j] =
          D[recursion_depth].r_weight[i][j];
    }
  }
  return std::make_pair(final_output_depth,
                        output_size_so_far + D[recursion_depth].R);
}

void generate_circuit(long long *query, long long N, int query_count,
  ...
  for (int i = 0; i < gates_count.size(); ++i) {
    v.C.circuit[i + 1].bit_length =
        mylog(smallest_pow2_larger_or_equal_to(gates_count[i]));
    v.C.circuit[i + 1].gates = new gate[1LL << v.C.circuit[i + 1].bit_length];
  }
  v.C.circuit[gates_count.size() + 1].bit_length =
      mylog(smallest_pow2_larger_or_equal_to(query_count));
  v.C.circuit[gates_count.size() + 1].gates =
      new gate[1LL << v.C.circuit[gates_count.size() + 1].bit_length];
  ...
  v.C.circuit[2].weight_expander_C_mempool =
      new prime_field::field_element[cn * C[0].L];
  ...
  for (long long i = 0; i < C[0].R; ++i) {
    ...
    v.C.circuit[2].gates[i + N].src =
        &v.C.circuit[2].src_expander_C_mempool[C_mempool_ptr];
    v.C.circuit[2].gates[i + N].weight =
        &v.C.circuit[2].weight_expander_C_mempool[C_mempool_ptr];
  ...
  auto output_depth_output_size =
      generate_enc_circuit(C[0].R, N + C[0].R, 1, 2);
  // add final output
  long long final_output_depth = output_depth_output_size.first + 1;
  ...
  v.C.circuit[final_output_depth].weight_expander_D_mempool =
      new prime_field::field_element[dn * D[0].L];

  for (long long i = 0; i < D[0].R; ++i) {
    v.C.circuit[final_output_depth].gates[output_so_far + i].ty =
        gate_types::custom_linear_comb;
    v.C.circuit[final_output_depth].gates[output_so_far + i].parameter_length =
        D[0].r_neighbor[i].size();
    v.C.circuit[final_output_depth].gates[output_so_far + i].src =
        &v.C.circuit[final_output_depth].src_expander_D_mempool[D_mempool_ptr];
    v.C.circuit[final_output_depth].gates[output_so_far + i].weight =
        &v.C.circuit[final_output_depth]
             .weight_expander_D_mempool[D_mempool_ptr];
    D_mempool_ptr += D[0].r_neighbor[i].size();
    for (long long j = 0; j < D[0].r_neighbor[i].size(); ++j) {
      v.C.circuit[final_output_depth].gates[output_so_far + i].src[j] =
          D[0].r_neighbor[i][j] + N;
      v.C.circuit[final_output_depth].gates[output_so_far + i].weight[j] =
          D[0].r_weight[i][j];
    }
  }

  for (long long i = 0; i < query_count; ++i) {
    v.C.circuit[final_output_depth + 1].gates[i] =
        gate(gate_types::relay, query[i], 0);
  }
...
std::pair<prime_field::field_element, bool> tensor_product_protocol(
    prime_field::field_element *r0, prime_field::field_element *r1,
    long long size_r0, long long size_r1, long long N, __hhash_digest *com_mt) {
  ...
  prime_field::field_element *combined_codeword =
      new prime_field::field_element[codeword_size[0]];
  __hhash_digest *combined_codeword_hash =
      new __hhash_digest[N / column_size * 2];
  __hhash_digest *combined_codeword_mt =
      new __hhash_digest[N / column_size * 4];
  memset(combined_codeword, 0,
         sizeof(prime_field::field_element) * codeword_size[0]);
  for (int i = 0; i < column_size; ++i) {
    for (int j = 0; j < codeword_size[0]; ++j) {
      combined_codeword[j] =
          combined_codeword[j] + r0[i] * encoded_codeword[i][j];
    }
  }
  ...
  for (int i = 0; i < N / column_size * 2; ++i) {
    if (i < codeword_size[0])
      combined_codeword_hash[i] =
          merkle_tree::hash_single_field_element(combined_codeword[i]);
    else
      combined_codeword_hash[i] = merkle_tree::hash_single_field_element(zero);
  }

  // merkle commit to combined_codeword
  merkle_tree::merkle_tree_prover::create_tree(
      combined_codeword_hash, N / column_size * 2, combined_codeword_mt,
      sizeof(__hhash_digest), false);

  // prover construct the combined original message
  prime_field::field_element *combined_message =
      new prime_field::field_element[N];
  ...
  // check for encode
  {
    prime_field::field_element *test_codeword =
        new prime_field::field_element[N / column_size * 2];
    int test_codeword_size =
        encode(combined_message, test_codeword, N / column_size);
  ...
  for (int i = 0; i < query_count; ++i) {
    ...
    for (int j = 0; j < column_size / 2; ++j) {
      column_hash = merkle_tree::hash_double_field_element_merkle_damgard(
          encoded_codeword[2 * j][q], encoded_codeword[2 * j + 1][q],
          column_hash);
    }

    assert(merkle_tree::merkle_tree_verifier::verify_claim(
        com_mt[1], com_mt, column_hash, q, N / column_size * 2, visited_com,
        proof_size));
    assert(merkle_tree::merkle_tree_verifier::verify_claim(
        combined_codeword_mt[1], combined_codeword_mt,
        merkle_tree::hash_single_field_element(combined_codeword[q]), q,
        N / column_size * 2, visited_combined_com, proof_size));
...
std::pair<prime_field::field_element, bool>
open_and_verify(prime_field::field_element x, long long N,
                __hhash_digest *com_mt) {
  assert(N % column_size == 0);
...
void dfs(prime_field::field_element *dst, prime_field::field_element *r,
         int size, int depth, prime_field::field_element val) {
  if (size == 1) {
    *dst = val;
  } else {
    dfs(dst, r, size / 2, depth + 1,
        val * (prime_field::field_element(1) - r[depth]));
    dfs(dst + (size / 2), r, size / 2, depth + 1, val * r[depth]);
  }
}

std::pair<prime_field::field_element, bool>
open_and_verify(prime_field::field_element *r, int size_r, int N,
                __hhash_digest *com_mt) {
  assert(N % column_size == 0);
  prime_field::field_element *r0, *r1;
  ...
  dfs(r1, r + log_column_size, N / column_size, 0,
      prime_field::field_element(1));
